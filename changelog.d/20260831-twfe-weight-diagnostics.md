...
- **TWFE weight diagnostics** (port of Brantly Callaway's `twfeweights` R
  ...
  - Validation: rejects NaN / `-inf` cohort labels, covariate-adjusted fits
    ...
    is dropped (`did`'s first-period drop, or running out of comparison
    units), and under `control_group="not_yet_treated"` the CS estimands
    average over each cohort's available post periods (`aggte`).
