...
def _assert_close(actual, expected, label, se=None, se_frac=0.05):
    ...
    assert diff < tol, (
        f"{label}: expected {expected}, got {actual:.1f} " f"(diff={diff:.1f}, tol={tol:.1f})"
    )
...
def _compute_es_avg(result):
    ...
    es = {int(e): d["effect"] for e, d in result.event_study_effects.items() if int(e) >= 0}
...
class TestHRSReplication:
    ...
    def test_sample_selection_yields_expected_counts(self, hrs_data):
        ...
        assert sorted(hrs_data["time"].unique()) == [
            7,
            8,
            9,
            10,
    ...
    def test_se_diagnostic_comparison(self, edid_hrs_result):
        ...
        for (g, t), (_, cluster_se) in TABLE6_EDID.items():
            ...
            assert (
                np.isfinite(analytical_se) and analytical_se > 0
...
class TestCompustatMCValidation:
    ...
    def test_edid_has_lower_rmse_than_cs(self, ci_params, rho):
        ...
        mc = _run_mc_simulation(
            n_sims,
            rho=rho,
            seed=3000 + int(rho * 100),
            also_cs=True,
        )

        rmse_edid = np.sqrt(np.mean((mc["edid_estimates"] - _TRUE_ES_AVG_COMPUSTAT) ** 2))
        rmse_cs = np.sqrt(np.mean((mc["cs_estimates"] - _TRUE_ES_AVG_COMPUSTAT) ** 2))

        # EDiD should not be meaningfully worse than CS
        assert (
            rmse_edid <= rmse_cs * 1.15
        ...
        if rho == -0.5:
            assert rmse_edid < rmse_cs, (
                f"rho={rho}: Expected RMSE_edid < RMSE_cs, " f"got {rmse_edid:.4f} >= {rmse_cs:.4f}"
            )
    ...
    def test_efficiency_gain_increases_with_serial_correlation(self, ci_params):
        ...
        def rel_rmse(mc):
            rmse_e = np.sqrt(np.mean((mc["edid_estimates"] - _TRUE_ES_AVG_COMPUSTAT) ** 2))
            rmse_c = np.sqrt(np.mean((mc["cs_estimates"] - _TRUE_ES_AVG_COMPUSTAT) ** 2))
    ...
    def test_coverage_approximately_correct(self, ci_params):
        ...
        covered = sum(ci[0] <= true_overall <= ci[1] for ci in mc["edid_overall_ci"])
