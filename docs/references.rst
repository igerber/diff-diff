...
- **Caetano, C., Callaway, B., Payne, S., & Sant'Anna, H. (2026).** "Difference-in-differences with 'bad controls'." arXiv:2608.03881 (v2, with Supplementary Appendix v1). https://arxiv.org/abs/2608.03881

  Identification with covariates that treatment can affect. Approach 1 (parallel trends given the pre-treatment bad control, Theorem 1 / Proposition 3) is computable by passing the bad control in ``CallawaySantAnna`` / ``DMLDiD`` ``covariates`` on the panel lane; the covariate-unconfoundedness approach (Theorem 2) ships as the ``DMLDiD`` bad-control lane - ``fit(..., bad_control=, bad_control_covariates=)`` - with the Neyman-orthogonal doubly-robust score ``ccps_panel_score`` / ``ccps_panel_score_augmented`` (Equations 10-11, Algorithm 1) in ``diff_diff/_dr_scores.py`` and the per-cell ``ATT_X(g,t)`` pre-test (Remark 6). Paper review on file at ``docs/methodology/papers/caetano-2026-review.md`` (a deliberate exception to the published-source rule); black-box parity against the authors' GPL-3 R package ``badcontrols`` (executed only, never read) at ``benchmarks/R/generate_badcontrols_golden.R`` / ``tests/test_dml_did_bad_controls_parity.py``.
