...
`ACRT(d)` (a constant only shifts the B-spline intercept, which `dPsi` annihilates; on the intercept-
free saturated basis the same identity holds via `L·1 = 0` — see below); they differ only
...
3'. `beta = (Psi'Psi)^{-1} Psi' Delta_tilde_Y`, so `beta_j = mean_{D=d_j}(Delta_tilde_Y) = ATT(d_j)`
    (a per-level 2×2 DiD).
4'. `ATT(d_j) = beta_j`; `ACRT(d_j) = (L beta)_j` where `L` is the finite-difference operator:
    backward `[ATT(d_j) - ATT(d_{j-1})]/(d_j - d_{j-1})` for `j >= 2`, and a **forward** difference at
    the lowest level `d_1` (each row of `L` sums to 0). `ACRT^{glob} = mean_i ACRT(D_i)`.
...
control variance; `L·1 = 0` cancels it in ACRT). `reg`/`dr` again share `ACRT(d_j)` point AND SE
(the indicator partition-of-unity makes residuals augmentation-invariant, and `L·1 = 0`); only the
`ATT(d_j)` level differs. See Notes #5-#6.
...
- [x] Discrete treatment saturated regression (`treatment_type="discrete"`) — CGBS 2024 Eq. 4.1; per-level ATT(d_j) + finite-difference ACRT (forward at d_1). Multi-cohort heterogeneous dose support deferred (see Note #6).
