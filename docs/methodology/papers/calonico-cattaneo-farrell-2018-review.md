...
*Fixed-n (nonasymptotic) variance formulas (Section 3.1 - what the port's `lprobust_vce` implements):*
...
- **HCk weighting** (Section 3.1): HC0 = raw squared residuals; HC1 divides by `(n - 2tr(Q_p) + tr(Q_p'Q_p))/n`; HC2 by `(1 - Q_{p,ii})`; HC3 by `(1 - Q_{p,ii})^2`, where `Q_p = R_p Gamma_p^{-1} R_p' W_p / n` is the (kernel-weighted) projection matrix. For `sigma^2_rbc`-HCk "the corresponding estimators ... are the same, but with q in place of p" (p. 775) - i.e., q-fit residuals AND q-fit leverage `Q_q`; any nprobust/R parity deviation stays documented under the registry's existing `Note`/`Deviation` labels. "These estimators may perform better for small sample sizes." Theory uses HC0; all HCk share the rates.
