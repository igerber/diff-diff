...
| **estimatr `classical` intercept SE excluded from parity (SE-audit (d)).** Same documented `O(1/n)` projection/DOF deviation as the slope; reference-only. Waived with the SE-audit row closure. | `tests/test_fixest_did_twfe_parity.py` | SE-audit / 2026-07-09 |
