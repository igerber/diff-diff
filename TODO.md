...
|----------|----------|----------|
| StackedDiD intercept SEs (`se_cr1/cr2_intercept`, SE-audit C1/(c)) — measured ~0.3% divergence from R: a nuisance-parameter reference-cell/parameterization gap, not machine-precision lockable (the event-study interaction SEs already match ~2e-13). Surfacing the field would add an unasserted, R-divergent public value; waived 2026-07-09 with the SE-audit row closure. | `tests/test_methodology_stacked_did.py` | SE-audit | Low |
| estimatr `classical` intercept SE (SE-audit (d)) — same documented `O(1/n)` projection/DOF deviation as the slope; reference-only, excluded from parity. Waived 2026-07-09 with the SE-audit row closure. | `tests/test_fixest_did_twfe_parity.py` | SE-audit | Low |
